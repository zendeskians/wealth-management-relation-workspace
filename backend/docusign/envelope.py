...
class Envelope:
    ...
    def send(envelope, session):
        ...
        results = envelope_api.create_envelope(account_id, envelope_definition=envelope)
        return results.envelope_id

    @staticmethod
    def get_view(
        envelope_id, envelope_args, client, session, authentication_method="None"
    ):
    ...
    def list(envelope_args, user_documents, session):
        ...
        envelopes_info = envelope_api.list_status_changes(
            account_id, from_date=envelope_args["from_date"], include="recipients"
        )
        if not envelopes_info.envelopes:
            return []
        results = [
            env.to_dict()
            for env in envelopes_info.envelopes
    ...
    def download(args, session):
        ...
        return send_from_directory(
            directory=dirname, filename=filename, as_attachment=True
        )
