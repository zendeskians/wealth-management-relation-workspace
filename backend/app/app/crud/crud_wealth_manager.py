...
class CRUDWealthManager(CRUDBase[Wealth_Manager, Wealth_ManagerCreate, Wealth_ManagerUpdate]):
    ...
    def get_by_username(self, db: Session, *, username: str) -> Optional[Wealth_Manager]:
        return db.query(Wealth_Manager).filter(Wealth_Manager.username == username).first()

    def get_by_id(self, db: Session, *, id: int) -> Optional[Wealth_Manager]:
        return db.query(Wealth_Manager).filter(Wealth_Manager.username == id).first()
    ...
    def update(
        self, db: Session, *, db_obj: Wealth_Manager, obj_in: Union[Wealth_Manager, Dict[str, Any]]
    ) -> Wealth_Manager:
    ...
    def authenticate(self, db: Session, *, email: str, password: str) -> Optional[Wealth_Manager]:
