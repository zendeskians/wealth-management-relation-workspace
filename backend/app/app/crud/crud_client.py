...
from app.crud.crud_wealth_manager import wealth_manager
...
class CRUDClient(CRUDBase[Client, ClientCreate, ClientUpdate]):
    ...
    def update(
        ...
        if update_data["wealth_manager_id"]:
            wealth_manager_id = wealth_manager.get_by_id(db, id=update_data["wealth_manager_id"])
            if not wealth_manager_id:
                return None
        return super().update(db, db_obj=db_obj, obj_in=update_data)

    def authenticate(self, db: Session, *, email: str, password: str) -> Optional[Client]:
